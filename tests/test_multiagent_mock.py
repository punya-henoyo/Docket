"""M4 harness check WITHOUT a live LLM: a scripted root spawns scripted sqli/cmdi
specialists through the REAL create_agent/wait_for_agents/AgentCoordinator/
spawn_child_agent pipeline. Every tool call actually executes (real HTTP against a
live vulnshop, real Finding registration/dedup) — only each agent's next-tool-call
decision is scripted. Proves: multi-agent spawn, concurrent execution (both children
run through http_request's asyncio.to_thread offload without blocking each other),
parent/child wiring, and root's aggregation of children's finding IDs.
...
ROOT_SCRIPT = [
    ...
    # Both children are typically done by the time this first wait_for_agents call
    # actually blocks (asyncio.wait's FIRST_COMPLETED returns everything already
    # done, not just one) — but script a second call defensively in case only one
    # had finished, matching the prompt's own "issue a wait, react, repeat" pattern.
    ("wait_for_agents", {}),
    ("wait_for_agents", {}),
    ("finish_scan", lambda outputs: {
        "summary": "sqli and cmdi specialists both reported.",
...
def _collect_findings(outputs: dict) -> list[str]:
    ...
    wait_for_agents reports the full current picture every call, including agents
    already-finished on a prior call — dedupe via a set rather than assuming each
    call contributes disjoint IDs."""
    ids: set[str] = set()
...
def _model_override(role: str) -> ScriptedModel:
    scripts = {"root": ROOT_SCRIPT, "sqli": SQLI_CHILD_SCRIPT, "cmdi": CMDI_CHILD_SCRIPT}
    return ScriptedModel(scripts[role])


def test_root_spawns_sqli_and_cmdi_specialists() -> None:
    store = FindingStore()
    import os
    ...
    assert result.finding_count == 2, result
    assert result.agents_spawned == 3  # root + sqli + cmdi
    assert len(store) == 2
...
if __name__ == "__main__":
    test_root_spawns_sqli_and_cmdi_specialists()
    print("test_multiagent_mock: ok — root spawned sqli+cmdi concurrently, both confirmed and aggregated")
